...
import type { KeyStat, MlsAdvancedMetrics, MlsGameState, SimulatorPlugin } from '../types';
...
const g:MlsGameState={sport:'mls',homeTeam:'MIA',awayTeam:'SEA',scoreHome:0,scoreAway:0,period:1,periodLabel:'1H',clockSeconds:HALF_SECONDS,possession:null,lastEvent:'Kickoff from the center circle.',keyStats:[],lastPlay:{type:'kickoff',description:'Kickoff from the center circle.',playId:0,progressivePasses:0,possessionPctHome:50,ppdaApprox:10,pressures:0,recoveries:0},matchClock:0,stoppageTime:0,possessionPctHome:50,shotsHome:0,shotsAway:0,shotsOnTargetHome:0,shotsOnTargetAway:0,xGHome:0,xGAway:0,cornersHome:0,cornersAway:0,passesCompletedPctHome:86,passesCompletedPctAway:85,foulsHome:0,foulsAway:0,yellowCardsHome:0,yellowCardsAway:0,redCardsHome:0,redCardsAway:0,bigChancesHome:0,bigChancesAway:0,goalkeeperSavesHome:0,goalkeeperSavesAway:0,advancedMetrics:{xG:0,xA:0,ppda:10,fieldTilt:50,progressivePasses:0,packing:0,ballRecoveries:0,pressures:0,shotEndingSequences:0,bigChancesCreated:0},boxScore,consistencyIssues:[]};
...
forceActions:['goal','red-card'],
forcePlay:(game,ctx,history,action)=>{
 if(action==='goal') return mlsSimulator.step(game,{...ctx,random:()=>0.6,randomInt:(a,b)=>b},history);
 if(action==='red-card') return mlsSimulator.step(game,{...ctx,random:()=>0.4,randomInt:(a,b)=>a},history);
 return mlsSimulator.step(game,ctx,history);
},
step:(previous,ctx)=>{const g=structuredClone(previous) as MlsGameState; const tick=ctx.randomInt(12,34);
const meta=g.boxScore!.meta??{};
let progressivePassesInc=0; let pressuresInc=0; let recoveriesInc=0; let xAInc=0; let packingInc=0; let shotSeqInc=0; g.clockSeconds=Math.max(0,g.clockSeconds-tick); g.matchClock+=tick; const atkHome=ctx.random()<0.5; const team=atkHome?'home':'away'; const players=atkHome?home:away; const fwd=players[ctx.randomInt(0,1)], mid=players[2], def=players[3], gk=players[4];
...
if(r<0.1){type='offside'; summary='Offside flag goes up.'; pressuresInc += 1;}
...
else if(r<0.3){type='tackle'; const other=atkHome?'away':'home'; const d=(other==='home'?home:away)[3]; bumpPlayerStat(g.boxScore!,other,d,'tackles',1); summary=`${d} wins a tackle.`; pressuresInc += 1; recoveriesInc += 1;}
else if(r<0.25){type='interception'; const other=atkHome?'away':'home'; const d=(other==='home'?home:away)[3]; bumpPlayerStat(g.boxScore!,other,d,'interceptions',1); summary=`Interception by ${d}.`; recoveriesInc += 1;}
else if(r<0.35){type='corner'; atkHome?g.cornersHome++:g.cornersAway++; bumpPlayerStat(g.boxScore!,team,mid,'corners',1); summary=`Corner for ${atkHome?g.homeTeam:g.awayTeam}.`; progressivePassesInc += 1;}
...
else { bumpPlayerStat(g.boxScore!,team,fwd,'shots',1); shotSeqInc += 1; progressivePassesInc += ctx.randomInt(0,2); atkHome?g.shotsHome++:g.shotsAway++; bumpPlayerStat(g.boxScore!,team,mid,'keyPasses',1);
if(ctx.random()<0.58){ xAInc += 0.08 + ctx.random()*0.12; bumpPlayerStat(g.boxScore!,team,fwd,'shotsOnTarget',1); atkHome?g.shotsOnTargetHome++:g.shotsOnTargetAway++; if(ctx.random()<0.34){type='goal'; packingInc += 1.2; bumpPlayerStat(g.boxScore!,team,fwd,'goals',1); bumpPlayerStat(g.boxScore!,team,mid,'assists',1); atkHome?g.scoreHome++:g.scoreAway++; atkHome?g.bigChancesHome++:g.bigChancesAway++; summary=`GOAL by ${fwd}, assisted by ${mid}.`; } else {type='save'; const other=atkHome?'away':'home'; bumpPlayerStat(g.boxScore!,other,(other==='home'?home:away)[4],'saves',1); atkHome?g.goalkeeperSavesAway++:g.goalkeeperSavesHome++; summary=`Save by ${other==='home'?g.homeTeam:g.awayTeam} keeper.`;}}
...
const totalDefActions = (g.boxScore!.teamTotals.away.tackles ?? 0) + (g.boxScore!.teamTotals.away.interceptions ?? 0);
const oppPasses = g.boxScore!.teamTotals.home.passComp ?? 0;
const ppda = oppPasses / Math.max(totalDefActions, 1);
const fieldTilt = clamp(45 + ((g.boxScore!.teamTotals.home.finalThirdEntries ?? 0) - (g.boxScore!.teamTotals.away.finalThirdEntries ?? 0)) * 1.6, 20, 80);
const progressive = Number(meta.progressivePasses ?? 0) + progressivePassesInc;
const pressures = Number(meta.pressures ?? 0) + pressuresInc;
const recoveries = Number(meta.recoveries ?? 0) + recoveriesInc;
const packing = Number(meta.packing ?? 0) + packingInc;
const shotSequences = Number(meta.shotSequences ?? 0) + shotSeqInc;
const xATotal = Number(meta.xATotal ?? 0) + xAInc;
g.boxScore!.meta = { ...meta, progressivePasses: progressive, pressures, recoveries, packing, shotSequences, xATotal };
g.advancedMetrics = {
  xG: Number(g.xGHome.toFixed(2)),
  xA: Number(xATotal.toFixed(2)),
  ppda: Number(ppda.toFixed(2)),
  fieldTilt: Number(fieldTilt.toFixed(1)),
  progressivePasses: progressive,
  packing: Number(packing.toFixed(1)),
  ballRecoveries: recoveries,
  pressures,
  shotEndingSequences: shotSequences,
  bigChancesCreated: g.bigChancesHome,
} satisfies MlsAdvancedMetrics;
...
g.lastEvent=summary; g.lastPlay={type,description:summary,playId:ctx.nextId(),player:fwd,assister:mid,xg:Number((0.03+ctx.random()*0.4).toFixed(2)),xa:Number((0.02+ctx.random()*0.2).toFixed(2)),shotType:ctx.random()<0.7?'open-play':'set-piece',keyPass:type==='goal'||type==='save'||type==='shot-miss',progressivePasses:ctx.randomInt(0,3),possessionPctHome:g.possessionPctHome,ppdaApprox:Number((8+ctx.random()*6).toFixed(1)),pressures:ctx.randomInt(0,2),recoveries:ctx.randomInt(0,2),isGoal:type==='goal'}; g.keyStats=buildKeyStats(g);
