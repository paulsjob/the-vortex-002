...
export const nhlSimulator: SimulatorPlugin={
  key:'nhl',label:'NHL',
  createInitialGame:()=>{const boxScore=initializeBoxScore('nhl',[...homeSkaters,goalies.home],[...awaySkaters,goalies.away],['goals','assists','sog','plusMinus','blocks','takeaways','toi','goalieSaves','goalieShotsAgainst','goalieGoalsAgainst','penalties']);
  ...
  step:(previous,ctx)=>{const g=structuredClone(previous) as NhlGameState; g.clockSeconds=Math.max(0,g.clockSeconds-ctx.randomInt(8,30)); if(g.ppTimeRemaining>0){g.ppTimeRemaining=Math.max(0,g.ppTimeRemaining-ctx.randomInt(8,20)); if(g.ppTimeRemaining===0) g.strengthState='EV';}
    ...
    const r=ctx.random();
    if(r<0.18){ type='penalty'; summary=`Minor penalty on ${attackHome?g.homeTeam:g.awayTeam}.`; bumpPlayerStat(g.boxScore!,atk,shooter,'penalties',1); g.strengthState=attackHome?'PK':'PP'; g.ppTimeRemaining=120; }
    else if(r<0.62){ type='shot-save'; summary=`Shot by ${shooter}, save made.`; if(attackHome){g.shotsHome++;g.sogHome++; bumpPlayerStat(g.boxScore!,'home',shooter,'sog',1); bumpPlayerStat(g.boxScore!,'away',goalies.away,'goalieSaves',1); bumpPlayerStat(g.boxScore!,'away',goalies.away,'goalieShotsAgainst',1); g.goalieSavesAway++; g.xGHome=Number((g.xGHome+0.08+ctx.random()*0.1).toFixed(2));}
    ...
    else if(r<0.83){ type='block'; const blocker=(attackHome?awaySkaters:homeSkaters)[ctx.randomInt(0,2)]; bumpPlayerStat(g.boxScore!,attackHome?'away':'home',blocker,'blocks',1); summary=`${blocker} blocks the attempt.`; if(attackHome)g.shotsHome++; else g.shotsAway++; }
